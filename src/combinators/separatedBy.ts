  ...
  new Parser((state: ParserState): ParserState => {
    ...
    if (valueParserState.isError) {
      return updateParserError(
        state,
        `separatedBy: value parser failed to match at offset ${valueParserState.offset}`,
      );
    }
    //#endregion
    ...
    if (separatorParserState.isError) {
      return updateParserError(
        state,
        `separatedBy: separator parser failed to match at offset ${separatorParserState.offset}`,
      );
    }
    //#endregion
